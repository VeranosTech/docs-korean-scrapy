...
.. class:: Crawler(spidercls, settings)
    ...
    .. attribute:: stats
        ...
        확장 모듈이나 미들웨어에서 다른 확장 모듈이 수집한 통계에 접근하거나
        자기 자신에 대한 통계를 저장하는데 사용된다.

        통계 수집기에 대한 소개는 :ref:`topics-stats`\ 를 참조한다.

        API에 대해서는 :class:`~scrapy.statscollectors.StatsCollector`\ 를 참조한다.
